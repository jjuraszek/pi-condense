...
└── src/
    ...
    ├── config.ts                  # Load/save <agent-dir>/settings.json `contextPrune` key (honors PI_CODING_AGENT_DIR)
    ...
    ├── indexer.ts                 # Runtime Map<toolCallId, ToolCallRecord> + session persistence + dedup-alias map
    ├── content-hash.ts            # SHA-1 + conservative normalization driving the pre-flush dedup pass
...
- **`capturePendingBatches(ctx)`** — extracted helper that runs the capture + trim + group steps from `flushPending` without any LLM work. Exposed to `commands.ts` via `registerCommands` so `/pruner now` can preview the pending queue and know the batch count before opening the `startPrunerWidget` progress widget.
- **`flushPending(ctx, options?)`** — summarizes + indexes all unsummarized batches in parallel by default, or sequentially (one call per batch) when `options.onProgress` is provided. Accepts `FlushOptions`: `{ delivery?, onProgress?, onBatchTextProgress?, previewedBatches? }`. When `previewedBatches` is set the internal capture step is skipped (avoids double-capture from `/pruner now`). While summarization is running, `onBatchTextProgress` receives the streamed summary character count for the active batch so callers can surface live progress in richer UIs (the `/pruner now` overlay and `context_prune` tool updates), while the footer itself stays at the simpler `prune: summarizing…` state. Before any LLM call, the flush runs a **pre-flush content-hash dedup pass** when `config.dedupByContentHash` is on: each tool call whose `(toolName, normalize(resultText))` hash already exists in the indexer is registered as a dedup alias via `indexer.registerDuplicate` (persisted as `CUSTOM_TYPE_DEDUP_ALIAS`) and removed from the batch. Fully-deduped batches are then classified against `config.minBatchChars`: batches whose remaining raw `resultText` is below the threshold are marked trivial, never sent to the summarizer, and reported back to `onProgress` with stage `"skipped"`; both kinds of skip still count toward the frontier advance so the same tool calls are not reconsidered next flush. The flush returns `reason: "skipped-trivial"` when every processed batch was trivial, `"skipped-oversized"` when every processed batch went through the LLM and was rejected as oversized, `"skipped-deduped"` when every processed batch was fully dedup'd, or `"flushed"` when at least one batch was actually summarized. Returns a typed `FlushResult` (with an optional `dedupedCount`).
...
- **`session_start`** — loads config from `<agent-dir>/settings.json` under the top-level `contextPrune` key (where `<agent-dir>` honors `PI_CODING_AGENT_DIR`), rebuilds the in-memory index and stats accumulator, clears `pendingBatches`, updates the footer status widget, calls `syncToolActivation()`, and notifies the user of the loaded state.
...
- **`turn_end`** — captures the batch (filtering out any `context_prune` tool call AND any tool whose name is in `config.protectedTools`, so protected outputs stay verbatim in context), pushes to `pendingBatches`. Behavior depends on `pruneOn` mode:
...
- **`ContextPruneConfig`** — `{ enabled, showPruneStatusLine, summarizerModel, summarizerThinking, pruneOn, remindUnprunedCount, batchingMode, quietOversizedSkips, minBatchChars, protectedTools, dedupByContentHash }` stored in `<agent-dir>/settings.json` under the top-level `contextPrune` key (per-agent-dir; honors `PI_CODING_AGENT_DIR`). `protectedTools: string[]` is the never-pruned allowlist; tool calls whose `toolName` appears in this list are filtered out at capture time so their raw `ToolResultMessage` stays verbatim in context, and they are excluded from the agentic-auto unpruned-count reminder. `minBatchChars: number` (default `1000`) is a pre-flush guard: batches whose total raw `resultText` is below this many chars are skipped without any summarizer LLM call, the frontier still advances, and the original tool results stay verbatim in context; `0` disables the guard. `dedupByContentHash: boolean` (default `true`) enables a SHA-1 pre-flush dedup pass: tool calls whose `(toolName, normalize(resultText))` matches a record already in the indexer are registered as aliases of the original via `CUSTOM_TYPE_DEDUP_ALIAS`, never reach the summarizer, and get stub-replaced by `pruneMessages` using the original's short ref. `PruneFrontierOutcome` adds `"skipped-deduped"`.
...
- Constants: `CUSTOM_TYPE_SUMMARY`, `CUSTOM_TYPE_INDEX`, `CUSTOM_TYPE_STATS`, `CUSTOM_TYPE_FRONTIER`, `CUSTOM_TYPE_DEDUP_ALIAS`, `STATUS_WIDGET_ID`, `DEFAULT_CONFIG`, `CONTEXT_PRUNE_TOOL_NAME`, `AGENTIC_AUTO_SYSTEM_PROMPT`.

### `src/config.ts` — Config persistence
- **`SETTINGS_PATH`** — constant resolving to `<agent-dir>/settings.json` (pi's main settings file, shared with pi's own `theme`, `defaultProvider`, `compaction`, etc.). `<agent-dir>` comes from pi's `getAgentDir()` (`@mariozechner/pi-coding-agent`): `$PI_CODING_AGENT_DIR` if set (tilde-expanded), else `~/.pi/agent`. Project-independent but scoped per agent-dir so each pi preset has its own settings (including its own `summarizerModel`).
- **`SETTINGS_KEY`** — the top-level JSON key (`"contextPrune"`) under which all extension state lives in `settings.json`. Pi's SettingsManager preserves unknown top-level keys when it rewrites the file, so this namespace coexists safely with pi's own settings.
- **`loadConfig()`** — reads `SETTINGS_PATH`, drills into the `contextPrune` key, normalizes each field (rejecting wrong types), and merges with `DEFAULT_CONFIG`. Returns defaults on missing file / parse error / missing key.
- **`saveConfig(config)`** — reads the current settings.json, merges in the `contextPrune` key with the new config, and writes the result back atomically (tmp file + `rename`) so concurrent pi writes (e.g. theme changes) can't observe a partial file.
...
Maintains the runtime `Map<toolCallId, ToolCallRecord>` and handles session persistence. Also owns the content-hash dedup state introduced by the pre-flush dedup pass:
- **`contentHashToOriginal: Map<hash, originalToolCallId>`** — populated by `addBatch` and `reconstructFromSession`. Drives the cheap pre-flush lookup.
- **`dedupAliasToOriginal: Map<dupToolCallId, originalToolCallId>`** — populated by `registerDuplicate` and rebuilt from `CUSTOM_TYPE_DEDUP_ALIAS` entries; consulted by `isSummarized` and `resolveToolCallId` so dup IDs route to the original record.
- **`reconstructFromSession(ctx)`** — scans the current branch's session entries for `CUSTOM_TYPE_INDEX`, `CUSTOM_TYPE_SUMMARY`, and `CUSTOM_TYPE_DEDUP_ALIAS`. Rebuilds the index, the short-ref maps, the content-hash dedup map (hashing each record at replay time), and the dedup alias map. Two-phase walk so dedup aliases land after their original short refs.
- **`addBatch(batch, appendEntry)`** — adds all records from a batch to the map, hashes each record into `contentHashToOriginal` (first-seen wins), and calls the supplied `appendEntry(CUSTOM_TYPE_INDEX, ...)` callback. The callback indirection lets `index.ts` route the append through either `pi.appendEntry` (runtime delivery) or `ctx.sessionManager.appendCustomEntry` (session delivery).
- **`lookupByContent(toolName, resultText)`** — returns the already-indexed toolCallId whose `(toolName, normalize(resultText))` hash matches, or `undefined`. Used by `flushPending`'s pre-flush dedup pass.
- **`registerDuplicate(newToolCallId, originalToolCallId, appendEntry)`** — registers an alias entry: extends `dedupAliasToOriginal`, reuses the original's short ref in `toolCallIdToAlias`, and persists `CUSTOM_TYPE_DEDUP_ALIAS` via the supplied delivery callback.
- **`isSummarized(toolCallId)`** — true when the id is in `this.index` OR in `dedupAliasToOriginal`; the pruner uses this for both real records and aliases.
- **`resolveToolCallId(id)`** — resolves short refs and dedup aliases; `getRecord` / `lookupToolCalls` go through this so the query tool returns the original record for dup'd ids too.

### `src/pruner.ts` — Context message filter
- **`pruneMessages(messages, indexer)`** — transforms the `context` event's message array. For each `ToolResultMessage` whose `toolCallId` is summarized, replaces the message with a small stub `[Summarized in pruner summary, ref \`tN\`. Use context_tree_query to retrieve full output.]` carrying `role: "toolResult"`, `isError: false`, and the original `toolCallId` / `toolName` / `timestamp`. This preserves role alternation so pi-ai's `transformMessages.insertSyntheticToolResults` does NOT inject a synthetic `{ isError: true, "No result provided" }` for the (no-longer-)orphaned tool call. Returns `{ messages, pruned }`: `pruned === true` means at least one stub-replacement happened and `messages` is a fresh array; `pruned === false` means no toolResult matched and `messages` is the original input reference so the caller can skip the reconstruction path. The `tN` short ref comes from `indexer.getShortRefForToolCallId(toolCallId)` and falls back to the raw `toolCallId` for legacy summaries that pre-date the short-ref system. All other messages (including `AssistantMessage` tool-call blocks that carry the IDs) are kept unchanged so the model can still reference them when calling `context_tree_query`.
...
- **`formatCharProgress(receivedChars, rawChars?)`** — formats live char progress like `1.2k summary chars / 8.4k raw chars`; the raw-char tail is omitted when `rawChars` is not supplied. Used by `/pruner now` widget rows and `pruneProgressText`.
...
- **`/pruner now`** — previews the pending batch queue via `capturePendingBatches(ctx)` and exits early with a notification if nothing is pending. Otherwise calls `startPrunerWidget(ctx, batches)` which registers a multi-row progress widget above the editor via `ctx.ui.setWidget(PROGRESS_WIDGET_ID, factory, { placement: "aboveEditor" })`. The widget pre-builds one row per batch carrying `{ label, toolCallCount, rawChars, status, receivedChars }`. While any row is in the `running` state a small `setInterval` re-requests render every 120 ms so the braille spinner (`⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏`) keeps animating even before the summarizer streams its first chunk. Row status is one of `pending` (○), `running` (spinner + live char progress), `done` (✓), or `skipped` (⚠). `flushPending` is invoked with `previewedBatches`, `onProgress`, and `onBatchTextProgress`, where `onProgress` flips rows between `running` / `done` / `skipped` and `onBatchTextProgress` streams the received-summary-character count into the active row. `startPrunerWidget` returns `{ updateRow, clearWidget }`; `clearWidget` stops the animation timer and removes the widget via `ctx.ui.setWidget(PROGRESS_WIDGET_ID, undefined)` once the flush completes, after which the normal footer status is restored.
...
| Config in `<agent-dir>/settings.json` under the `contextPrune` key (agent-dir honors `PI_CODING_AGENT_DIR`) | Shares pi's own settings file so the user has a single source of truth and pi's settings UI / theme picker won't fight us. Atomic tmp-file + rename writes keep concurrent writes safe; namespacing under `contextPrune` mirrors pi's own conventions (`compaction`, `retry`, `branchSummary`, …). Config persists across all projects within an agent-dir, and each pi preset directory gets its own config (different summarizer model per preset). |
...
| `startPrunerWidget` (above-editor widget) for `/pruner now` | Renders one progress row per batch in a non-modal `aboveEditor` widget instead of a blocking overlay. Each row shows a braille spinner driven by a 120 ms `setInterval`, the running batch's live summary-character count via `formatCharProgress`, and per-batch raw-char totals so the user can see how much context will be reclaimed. Status transitions to ✓ on done and ⚠ on skipped; the widget is torn down via `ctx.ui.setWidget(PROGRESS_WIDGET_ID, undefined)` when the flush returns. Sequential LLM calls are used only for this user-triggered path; auto-flush paths keep the parallel per-batch approach for efficiency. |
...
| `capturePendingBatches` extracted from `flushPending` | Lets `/pruner now` preview the queue before opening the widget so it knows how many rows to pre-build. Avoids double-capture via `previewedBatches` option on `flushPending` |
...
| Pre-flush content-hash dedup | Catches re-reads of already-indexed tool outputs at zero LLM cost. Hash is SHA-1 of `(toolName, "\0", normalize(resultText))` where normalization is line endings + per-line trailing whitespace + `trim()` only. Duplicates reuse the original's short ref (Option B alias) so `pruneMessages` emits a consistent `tN` for both. Persisted via `CUSTOM_TYPE_DEDUP_ALIAS` so reconstruction works across restarts and branch switches. V1 only dedupes against the indexer (cross-flush); intra-flush dedup is deferred so we never produce an alias pointing at a never-indexed canonical. |
